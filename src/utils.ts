 ...
 */
	...
	static createNewServer(typeServer, serverName, port, address, buildVersion, secure, includes): string | undefined {
		...
		if (serverConfig.configurations) {
			...
			if (servers.find(element => { return element.name === serverName; })) {
				vscode.window.showErrorMessage("Server name already exists");
				return undefined;
			} else {
	...
	static removeExpiredAuthorization() {
		vscode.window.showWarningMessage('Expired authorization token deleted');
			...
			if (absolutePath) {
				...
				includes.forEach((value, index, elements) => {
					...
					try {
						...
						if (!fi.isDirectory) {
							const msg: string = "Review the folder list in order to search for settings (.ch). Not recognized as folder: {0}"; //, value);
